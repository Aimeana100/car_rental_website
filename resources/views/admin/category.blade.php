...
<div class="content ">
    ...
    <div  class="row breadcrumb-container d-flex justify-content-between pr-30">
        ...
        <nav aria-label="breadcrumb">
            ...
            <ol class="breadcrumb bg-info ">
              ...
              <li class="breadcrumb-item text-dark active" aria-current="page"> Categories </li>
            </ol>

          </nav>
          <div class="mr-30">
            <a  href="#" data-toggle="modal" class="btn btn-primary btn-sm btn-flat btn-Add"><i class="fa fa-plus"></i> New Category</a>
          </div>
   </div>
  ...
  <div style="margin: 50px" class="container mt-30">
    ...
    <div class="row">
        ...
        <table id="CollegeTable" class="display nowrap mt-3 w-full w-100" style="width:800px !important">
            <thead>
                <tr>
                    <th>Id</th>
                    <th>Poster </th>
...
@endsection
@section('modals')
@include('admin.includes_modal.category_modal');
@endsection
@section('js')
<script>
var table;
        // './resources/**/*.vue',
var manageTable = $('#CollegeTable');

//  Data table population Function
// myFunc();
function myFunc() {
    var defaultUrl = "{{ route('admin.categories.generalList') }}";
            ...
            table = manageTable.DataTable({
                ...
                columns: [
                    ...
                    {
                        if(row.image){
                            return '<img width="90" src="../uploads/images/' + row.image + '">';
                        }
                        else{
                            return '<img width="90" src="../uploads/images/1640520398ciy3.jpg">';
...

                    {
                        ...
                        render: function (data, type, row) {
                            ...
                            return "<button class='btn btn-success btn-sm btn-flat js-edit' data-id='" + data +
                                "' data-url='/admin/categories/show/" + row.id + "'> <i class='fa fa-pen'></i>Edit</button>" +

                                "<button class='btn btn-danger btn-sm btn-flat js-change-status ' data-id='" + data +
                                "' data-url='/admin/categories/changeStatus/" + row.id + "'> <i class='fa fa-trash'></i>Change Status</button>";
...
$('.btn-Add').on('click', function(){
    ...
    manageTable.on('click', '.js-edit', function () {
                ...
                $("#edit_category_desc").val(response.category.description);
                if(response.category.image){
                $("#stamp-placeholder").attr( 'src', '../uploads/images/' + response.category.image);
                }
                else{
                            ...
                            $.ajax({
                               ...
                               form[0].reset();
                                ...
                                $('#update-messages').html('<div class="alert alert-success alert-success-update">' +
                                    ...
                                    '<strong><i class="glyphicon glyphicon-ok-sign"></i></strong> Category  successfully updated. </div>');
            ...
            manageTable.on('click', '.js-change-status', function()
            {
                ...
                $.ajax({
                    ...
                    success: function (response) {
                        ...
                        if(status == 1){
                var html = '<form id="frmChangeStatus" action="/admin/categories/changeStatus" method="POST" >'+
            ...
            // icon: 'info',
            html:
                '<form id="frmChangeStatus" action="admin/categories/Changestatus" method="POST" >'+
                ...
                $.ajax({
                    ...
                    if (data.result=="ok"){
                        ...
                        $('#add-messages').html('<div class="alert alert-success alert-message flat">' +
                            '<button type="button" class="close" data-dismiss="alert">&times;</button>' +
                            '<strong><i class="glyphicon glyphicon-ok-sign"></i></strong> College successfully saved. </div>');
