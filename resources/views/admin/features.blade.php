...
<div class="content ">
    ...
    <div  class="row breadcrumb-container d-flex justify-content-between pr-30">
        ...
        <nav aria-label="breadcrumb">
          ...
          <div class="mr-30">
            <a  href="#" data-toggle="modal" class="btn btn-primary btn-sm btn-flat btn-Add"><i class="fa fa-plus"></i> New Feature</a>
          </div>
   </div>
...
@endsection
@section('modals')
@include('admin.includes_modal.feature_modal');
@endsection
@section('js')
<script>
var table;
...
// myFunc();
function myFunc() {
    var defaultUrl = "{{ route('admin.features.generalList') }}";
...

                    {
                        ...
                        render: function (data, type, row) {
                            ...
                            return "<button class='btn btn-success btn-sm btn-flat js-edit' data-id='" + data +
                                "' data-url='/admin/features/show/" + row.id + "'> <i class='fa fa-pen'></i>Edit</button>" +

                                "<button class='btn btn-danger btn-sm btn-flat js-change-status ' data-id='" + data +
                                "' data-url='/admin/features/changeStatus/" + row.id + "'> <i class='fa fa-trash'></i>Change Status</button>";
...
$('.btn-Add').on('click', function(){
    ...
    manageTable.on('click', '.js-edit', function () {
                            ...
                            $.ajax({
                               ...
                               form[0].reset();
                                ...
                                $('#update-messages').html('<div class="alert alert-success alert-success-update">' +
                                    ...
                                    '<strong><i class="glyphicon glyphicon-ok-sign"></i></strong> Feature  successfully updated. </div>');
            ...
            manageTable.on('click', '.js-change-status', function()
            {
                ...
                $.ajax({
                    url: '/admin/features/show/'+ id,
                    type: 'GET',
                    data: {id: id},
                    dataType: 'json',
                    success: function (response) {
                        var status = response.feature.status;
                        if(status == 1){
                var html = '<form id="frmChangeStatus" action="/admin/features/changeStatus" method="POST" >'+
                            ...
                            $.ajax({
                               ...
                               form[0].reset();
                                ...
                                $('#changeStatus-messages').html('<div class="alert alert-success alert-success-update">' +
                                    '<button type="button" class="close" data-dismiss="alert">&times;</button>' +
                                    '<strong><i class="glyphicon glyphicon-ok-sign"></i></strong> Event  successfully updated. </div>');
            ...
            // icon: 'info',
            html:
                '<form id="frmChangeStatus" action="admin/features/Changestatus" method="POST" >'+
